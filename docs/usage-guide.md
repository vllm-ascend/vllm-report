...
5. 完成后执行：
   python track_adaptation.py update --sha abc123 --status adapted
```
...
socat TCP-LISTEN:9999,reuseaddr,fork \
  EXEC:"python /path/to/vllm-report/mcp_server.py --data-dir /path/to/vllm-report/data --ascend-repo-path /path/to/vllm-ascend"
```
...
4. 标记 → python track_adaptation.py update --sha xxx --status adapted
```
...
| **邮件报告**（send_report.py） | 每日推送 | 不变，继续使用 |
| **MCP Server**（mcp_server.py） | Agent 查询入口 | 22 个 tools，知识库核心接口。支持渐进式加载——先调 `get_architecture_overview` 获取轻量概览，再按需深入具体维度 |
