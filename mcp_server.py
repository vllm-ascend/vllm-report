...
Usage:
  python mcp_server.py --data-dir /path/to/vllm-report/data --ascend-repo-path /path/to/vllm-ascend
...
async def tool_get_ascend_impact_summary(repo: str, date_from: Optional[str] = None, date_to: Optional[str] = None) -> str:
    ...
    tags_index = index.get("tags_index", {})
    sha_list = tags_index.get("ascend_affected", [])
    sha_set = set(sha_list)
    ...
    for repo_name in ["vllm", "vllm-ascend"]:
        ...
        if ascend_repo_path and repo_name == "vllm":
            ...
            if main_sha and commit_sha != "unknown":
                # Simple comparison: if arch SHA != baseline SHA, it's based on different code
                info["baseline_sha"] = main_sha[:12]
                info["matches_baseline"] = commit_sha[:12] == main_sha[:12]
...
async def tool_get_pending_adaptations() -> str:
    ...
    unknown = [c for c in adaptation.get("commits", []) if c.get("status") == "unknown"]
...
async def tool_update_adaptation_status(sha: str, status: str, notes: Optional[str] = None) -> str:
    ...
    return json.dumps({"success": True, "sha": sha, "status": status, "stats": stats}, ensure_ascii=False, indent=2)
...
TOOLS = [
    ...
    Tool(
        ...
        description="Update adaptation status for a commit",
        inputSchema={
            "type": "object",
            "properties": {
                "sha": {
                ...
                "status": {
                    "type": "string",
                    "enum": ["unknown", "pending", "in_progress", "adapted", "skipped"],
                    "description": "New status",
                },
                "notes": {
                    "type": "string",
                    "description": "Optional adaptation notes",
                },
            },
            "required": ["sha", "status"],
        },
